...
#'  }
#' @param regions.list A string vector with a list of full paths to bed files or data.frames in at least BED3 format (eg. generated by \link{build.bed}).
...
density.matrix = function(mode,
  ...
  import_bed = function(bed, mode, upstream, downstream, reference.point, sort.regions.coordinates) {
    ...
    if (class(regions) == "data.frame") {
      ...
      } else if (length(regions) >= 3 & length(regions) < 6) {
        if (length(regions) == 3) {
          regions =
            regions %>%
    ...
    names(regions) = c("chr", "start", "end", "name", "score", "strand")
